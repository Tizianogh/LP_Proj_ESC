...
export class CelebrationComponent implements OnInit {
    ...
    sendAll() {
        ...
        this.usersList.forEach(u => {
            if (!(this.mailList.includes(u['email']))) {
                this.mailList.push(u['email']);
            }
        });

        var userF = this.usersList.find(u => u['userId'] == this.election['electedId']);
        for (let i in this.mailList) {
            this.service.post(window.location.origin + "/api/sendmail", {
                "To": this.mailList[i],
                "Cc": "",
                "Subject": "Fin de l'election " + this.election['job'],
                "Text":
