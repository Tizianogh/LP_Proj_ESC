...
export class ElectionReminderComponent implements OnInit {
    ...
    addDest(mail: HTMLInputElement) {
        if (this.regexMail.test(mail.value) && !(this.mailList.includes(mail.value))) {
            this.mailList.push(mail.value);
            (document.querySelector("#mailInput") as HTMLInputElement).style.borderColor = '#ced4da';
            (document.querySelector("#mailInput") as HTMLInputElement).value = "";
        }
        else {
    ...
    deleteItem(mail: string) {
        for (let i = 0; i <= this.mailList.length; i++) {
            if (mail == this.mailList[i]) {
    ...
    sendAll() {
        ...
        for (let i in this.mailList) {
            this.service.post(window.location.origin + "/api/sendmail", {
                "To": this.mailList[i],
                "Cc": "",
                "Subject": "Invitation pour l'election " + this.poste,
                "Text":
             ...
             <tr style="border-collapse:collapse;"> 
              <td align="left" style="Margin:0;padding-left:20px;padding-right:20px;padding-top:30px;padding-bottom:30px;background-position:left top;"> 
               <table width="100%" cellspacing="0" cellpadding="0" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;"> 
                ...
                let re = /\./gi;
                (document.querySelector('#' + this.mailList[i].split('@')[0].split('.').join('')) as HTMLElement).style.backgroundColor = "#00e676";
            }, error => {
                    console.log(error);
                    let re = /\./gi;
                    (document.querySelector('#' + this.mailList[i].split('@')[0].split('.').join('')) as HTMLElement).style.backgroundColor = "#ff0000";
            });
        }
    }
